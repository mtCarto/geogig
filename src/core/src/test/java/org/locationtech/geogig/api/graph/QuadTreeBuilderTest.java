...
@Ignore
public class QuadTreeBuilderTest extends TreeBuilderTest {
    ...
    protected TreeBuilder createBuiler() {
        return RevTreeBuilder.quadTree(objectStore);
    }

    @Override
    protected TreeBuilder createBuiler(RevTree original) {
        return RevTreeBuilder.quadTree(objectStore).original(original);
    }
    ...
    public void testSmallRects() throws IOException {
        ...
        TreeBuilder sequentialTree = createQuadTree(maxBounds, maxDepth, nodes);
        ...
        TreeBuilder randomOrderTree = createQuadTree(maxBounds, maxDepth, nodes);
    ...
    public void pointsPerformanceComparison() throws Exception {
        ...
        final RevTree canonicalTree;
        {
            ...
            Collections.sort(nodes, NodeStorageOrder.INSTANCE);// make it easier for the tree
            // builder
    ...
    public void diffQuadTreeTest() throws Exception {
        ...
        TreeBuilder origQuadTree = createQuadTree(maxBounds, maxDepth, nodes);
        ...
        TreeBuilder changedQhadTree = createQuadTree(maxBounds, maxDepth, nodes);
    ...
    private RevTree createRevTree(TreeBuilder quadTree) {
    ...
    private RevTree createCanonicalTree(List<Node> nodes, ObjectStore odb) {
        ...
        // builder = new RevTreeBuilder(odb);
        builder = RevTreeBuilder.canonical(odb);
        Stopwatch treeTime = Stopwatch.createStarted();
        for (Node n : nodes) {
            builder.put(n);
        }
        int depth = builder.depth();
        RevTree tree = builder.build();
        treeTime.stop();
        System.err.printf("Canonical RevTree (depth %d) created in %s: %s\n", depth, treeTime,
                tree);
        odb.put(tree);
        return tree;
    }
    ...
    private RevTreeBuilder createQuadTree(Envelope maxBounds, int maxDepth, final List<Node> nodes,
        ...
        RevTreeBuilder qtree = RevTreeBuilder.quadTree(objectStore, maxBounds, maxDepth);
    ...
    public static void main(String[] args) {
        ...
        TreeBuilder qtree = RevTreeBuilder.quadTree(odb, maxBounds, maxDepth);
