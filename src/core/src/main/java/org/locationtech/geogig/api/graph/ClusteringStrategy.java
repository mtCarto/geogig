...
public abstract class ClusteringStrategy {
    ...
    //
    protected abstract int maxBuckets(final int depthIndex);

    public abstract int normalizedSizeLimit(final int depthIndex);
    ...
    private synchronized int put(final TreeId dagId, final DAG dag, final NodeId nodeId,
        ...
        @Nullable
        SortedSet<NodeId> children = dag.children();
        ...
        if (children == null) {
            ...
            if (bucketId == null) {
                ...
                SortedSet<TreeId> buckets = dag.buckets();
            ...
            if (size > normalizedSizeLimit) {
                SortedSet<NodeId> nodeIds = dag.switchToBuckets();
    ...
    private void shrinkIfUnderflow(final DAG bucketsDAG, NodeId nodeId, int depth) {
        ...
        if (childCount <= normalizedSizeLimit && bucketsDAG.buckets() != null) {
            SortedSet<NodeId> childrenRecursive = getChildrenRecursive(bucketsDAG, nodeId, depth);
            checkState(childrenRecursive.size() == childCount, "expected %s, got %s", childCount,
                    childrenRecursive.size());
            final SortedSet<NodeId> children = bucketsDAG.switchToLeaf();
            children.addAll(childrenRecursive);
        }
    }

    private SortedSet<NodeId> getChildrenRecursive(final DAG dag, final NodeId nodeId,
            final int depth) {
        ...
        if (dag.buckets() != null) {
            for (TreeId bucketId : dag.buckets()) {
                ...
                SortedSet<NodeId> bucketChildren = getChildrenRecursive(bucket, nodeId, depth + 1);
            ...
            if (originalIsLeaf) {
                ...
                if (root.getState() == STATE.INITIALIZED) {
                    { // make DAG a bucket tree
                        SortedSet<NodeId> promotable = root.switchToBuckets();
    ...
    private void preload(ImmutableCollection<Bucket> values) {
        this.storageProvider.getTreeCache().preload(
                Iterables.transform(values, (b) -> b.getObjectId()));
    }
