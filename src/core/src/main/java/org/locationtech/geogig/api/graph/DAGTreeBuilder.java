...
class DAGTreeBuilder {
    ...
    static {
        ...
        ThreadFactory builderFactory = new ThreadFactoryBuilder()
                .setNameFormat("DAGTreeBuilder-%d").setDaemon(true).build();
        ...
        STORAGE_EXECUTOR = MoreExecutors.listeningDecorator(Executors.newFixedThreadPool(
    ...
    private static class SharedState {
        ...
        public SharedState saveTrees() {
            ...
            if (!toSave.isEmpty()) {
                ...
                // .getName(), toSave.size());
                ListenableFuture<?> future = STORAGE_EXECUTOR.submit(() -> targetStore
                        .putAll(toSave.iterator()));
                futures.add(future);
    ...
    private static class TreeBuildTask implements Callable<RevTree> {
        ...
        public RevTree call() {
            ...
            try {
                ...
                if (root.getState().equals(STATE.CHANGED)) {
                    ...
                    final @Nullable SortedSet<TreeId> dagBuckets = root.buckets();
        ...
        private RevTree buildBucketsTree(final DAG root) {
            ...
            final SortedSet<TreeId> dagBuckets = root.buckets();
            ...
            RevTree result = RevTreeImpl.create(size, childTreeCount, treeNodes, featureNodes,
                    buckets);
            return result;
        }

        private RevTree buildLeafTree(DAG root) {
            final SortedSet<NodeId> unpromotable = root.unpromotable();
        ...
        private RevTree buildLeafTree(ImmutableList<Node> promotable) {
            ...
            RevTree tree = RevTreeImpl.create(size, childTreeCount, treesList, featuresList,
                    buckets);
        ...
        private ImmutableList<Node> toNodes(final @Nullable SortedSet<NodeId> nodeIds) {

            ImmutableList.Builder<Node> nodes = ImmutableList.builder();

            if (nodeIds != null && !nodeIds.isEmpty()) {
                for (NodeId nodeId : nodeIds) {
                    Node node = state.clusteringStrategy.getNode(nodeId);
                    nodes.add(node);
                }
            }
